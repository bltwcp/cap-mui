...
test('normal MultiSelect', () => {
    render(_jsx(MultiSelect, { label: 'title', menuItems: ['a', 'b', 'c'], value: ['a'], placeholder: 'QQ', abbr: '...' }));
    expect(screen.getByText('title:')).toBeInTheDocument();
    expect(screen.getByText('QQ')).toBeInTheDocument();
});
test('MultiSelect tooltip test', () => {
    render(_jsx(MultiSelect, { label: 'title', menuItems: ['a', 'b', 'c'], value: ['a'], placeholder: 'QQ', abbr: '...', tooltip: 'verticle' }));
...
test('MultiSelect event test', () => {
    const mockSelectedFn = jest.fn((values) => values);
    render(_jsx(MultiSelect, { label: 'title', menuItems: ['a', 'b', 'c'], value: ['a'], placeholder: 'QQ', abbr: '...', onChange: (v) => mockSelectedFn }));
    fireEvent.mouseDown(screen.getByRole('button'));
...
test('MultiSelect with hierarchy options', () => {
    ...
    render(_jsx(MultiSelect, { menuItems: hierarchyItems }));
    // expand test
    fireEvent.mouseDown(screen.getByRole('button'));
    expect(screen.getByRole('presentation')).toBeInTheDocument();
