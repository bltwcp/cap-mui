...
test('normal Tabs', () => {
    const mockTabChangeEvent = jest.fn((index) => index);
    render(_jsx(Tabs, { tabs: ['tab a', 'tab b'], currentTab: 1, onTabChange: mockTabChangeEvent }));
...
test('closeabled Tabs', () => {
    const mockTabChange = jest.fn((index) => index);
    render(_jsx(Tabs, { currentTab: 2, tabs: ['tab a', 'tab b', 'tab c'], freezeTabs: 2, onTabChange: mockTabChange }));
...
test('user handle closeabled Tabs', () => {
    const mockTabClose = jest.fn((index) => index);
    render(_jsx(Tabs, { tabs: ['tab a', 'tab b', 'tab c'], freezeTabs: 2, onTabClose: mockTabClose }));
    fireEvent.click(screen.getByTestId('CloseIcon'));
