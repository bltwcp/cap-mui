...
import {
    PeriodType, DateInfoProps, now, nowSubFromPeriod, nowYearFromPeriod,
    datePicker, yearPicker, subPicker, dateTip, disabledSelect,
} from './utils'

const fetchWeekNoList = async (year: string) => {
    const { data } = await axios.get(`${process.env.REACT_APP_CAPMUI_BASEPATH}/Date/GetWeekNoOptionList?checkYear=${year}`)
    return data?.content ?? []
}

const useWeekNoList = (year: string) => useQuery(
    ['weeknolist', year],
    () => fetchWeekNoList(year),
    {
        staleTime: Infinity,
        retry: 0,
        select: (vs) => vs.map((v: number) => v.toString()),
    }
)
...
interface DateRangePickerProps {
    ...
    tipDate2StrFn?: (d: Date) => string | undefined
    comparison?: boolean
}

const DefaultPeriodTypes: PeriodType[] = ['Week', 'Month', 'Quarter', 'Year']
...
const defaultPeriodType = (period: PeriodType | undefined) => period ?? DefaultPeriodType
const defaultDateRange = (props: DateRangePickerProps): DateRangeType => {
...
const defaultSystemStartYear = (startYear: number | undefined) => startYear ?? 2000
const defaultSystemEndYear = () => now.year + 1
const defaultPeriod = (propsPeriods: PeriodType[], propsPeriod: PeriodType) => {
    if (propsPeriods.includes(propsPeriod))
        return propsPeriod
    return propsPeriods[0]
}

export const DateRangePicker = forwardRef((props: DateRangePickerProps, ref?: Ref<HTMLElement>) => {
    const { onRangeChange } = props
    const [period, setPeriod] = useState<PeriodType>(defaultPeriodType(props.defaultPeriod))
    const [dateRange, setDateRange] = useState<DateRangeType>(defaultDateRange(props))
    ...
    const handlePeriodChange = (newPeriod: string | undefined) => {
        const nextPeriod = (newPeriod ?? DefaultPeriodType) as PeriodType
    ...
    const handleFromYearChange = (fromYear: string) => {
    ...
    const handleFromSubdimensionChange = (fromSub: string) => {
    ...
    const handleToYearChange = (toYear: string) => {
    ...
    const handleToSubdimensionChange = (toSub: string) => {
    ...
    const fromDate = (): string | JSX.Element => {
    ...
    const toDate = (): string | JSX.Element => {
    ...
    return (<span ref={ref}>
        {validRange && (<>
            ...
            <Select
                menuItems={defaultperiods}
                initialSelected={defaultPeriod(defaultperiods, defaultperiod)}
                onChange={handlePeriodChange}
                disabled={disabled}
                selectedAtDisabled={periodAtDisabled}
                sx={{ width: '70px' }}
            />

            {leadingLabel('From')}
            {datePicker(period, dateRange.fromDate, disabled, handleFromDateChange)}
            {yearPicker(period, dateRange.fromYear, pickableYears, disabled,
                disabledSelect([props.disabledSelect?.FromYear, props.defaultFromYear, now.year.toString()]),
                handleFromYearChange)}
            {subPicker(period, dateRange.fromSub, pickableStartSubdimension, disabled,
                disabledSelect([props.disabledSelect?.FromSub, props.defaultFromSub, now[period.toLowerCase() as keyof DateInfoProps]?.toString()]),
                handleFromSubdimensionChange,
                fromDate
            )}
            
            {leadingLabel('To')}
            {datePicker(period, dateRange.toDate, disabled, handleToDateChange)}
            {yearPicker(period, dateRange.toYear, pickableYears, disabled,
                disabledSelect([props.disabledSelect?.ToYear, props.defaultToYear, now.year.toString()]),
                handleToYearChange)}
            {subPicker(period, dateRange.toSub, pickableEndSubdimension, disabled,
                disabledSelect([props.disabledSelect?.ToSub, props.defaultToSub, now[period.toLowerCase() as keyof DateInfoProps]?.toString()]),
                handleToSubdimensionChange,
                toDate
            )}
        </>)}
    </span>)
