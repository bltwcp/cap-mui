...
export const MemorizedGrid = forwardRef((props, ref) => {
    const [memoColumns, setMemoColumns] = useState(props.columns ?? []);
    const [memoRows, setMemoRows] = useState(props.rows ?? []);
    const [memoSx, setMemoSx] = useState(props.sx ?? {});
    const [memoBottom, setMemoBottom] = useState(props.bottom ?? 0);
    const [memoProgressSx, setMemoProgressSx] = useState(props.progressSx ?? {});
    const [memoIndex, setMemoIndex] = useState(props.index ?? DefaultIndex);
    const [memoFreezeRows, setMemoFreezeRows] = useState(props.freezeRows ?? DefaultFreezeRows);
    const [memoHierarchyColumns, setMemoHierarchyColumns] = useState(props.hierarchyColumns ?? []);
    const [memoEmptyDisplay, setMemoEmptyDisplay] = useState(props.emptyDisplay);
    useEffect(() => {
    ...
    return (_jsx(MemoGrid, { ...props, sx: memoSx, bottom: memoBottom, progressSx: memoProgressSx, columns: memoColumns, rows: memoRows, index: memoIndex, freezeRows: memoFreezeRows, emptyDisplay: memoEmptyDisplay, hierarchyColumns: memoHierarchyColumns }));
});
