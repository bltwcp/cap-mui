...
test('empty Grid with cust EmptyComponent', () => {
    render(_jsx(Grid, { emptyDisplay: _jsx("div", { "data-testid": 'no-content', children: "Empty" }) }));
...
test('Grid with data', () => {
    render(_jsx(Grid, { columns: cols1, rows: rows1 }));
...
test('Grid with indexes', () => {
    render(_jsx(Grid, { columns: cols1, rows: rows1, index: true }));
    const headerCells = screen.getAllByRole('columnheader');
...
test('Grid with hierarchy data', () => {
    render(_jsx(Grid, { index: true, columns: cols2, rows: rows2, hierarchyColumns: ['a'] }));
    expect(screen.getAllByRole('row').length).toBe(3);
...
test('show hide dialog', async () => {
    render(_jsx(Grid, { columns: cols2, rows: rows2 }));
